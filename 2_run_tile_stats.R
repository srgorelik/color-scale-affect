...
# system('gsutil -m cp etc/tiles_1000px_all.csv gs://uw-ks-data/harris30m/')
...
csv.out <- 'output/stats_all_tiles.csv'
cat(paste('Writing', csv.out, '...\n'))
write.csv(df.out, file = csv.out, row.names = F)
