...
#
# History:
#	Created by Seth Gorelik, 9/22/24
#
...
# replace with output from Color Crafter (https://danielleszafir.com/ColorCrafter/) (9 colors as hex codes)
...
inp.csv <- '~/repos/uw/results/map_values.csv'

# set output directory
out.dir <- '~/repos/uw/results/maps'
...
for (i in 1:nrow(df)) {
	...
	v <- as.numeric(df[i, -1])
	m <- matrix(data = v, nrow = 100, ncol = 100)
	mr <- apply(m, 1, rev)
	mrt <- t(mr)
...
### TESTING ###
# color.scale.spline <- colorRampPalette(color.scale.hex, space = 'Lab', interpolate = 'spline')
###############
